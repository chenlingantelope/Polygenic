...
mkdir $resultdir
> $resultdir/results.txt # initialise the output file

# Iterate across all possible parameters

for NC in 100 # nr of chromosomes
do
	for K in 20 50 # nr of loci
	do
		for CF in 0.5 0.6 0.7 0.8 # current alle freqs
		do
			for H in 0.6 0.7 0.8 0.9 # hereditability (h2)
			do
				for AF in 5 10 20 # effect size
				do
					for M in 0 # model, 0 is additive, only available now
					do
						for SC in 30 50 70 100 200 300 400 600 800 1000 # sel coeffs to test
						do
#NC=100;K=20;CF=0.8;H=0.6;AF=10;M=0;SC=400
							# initialize intermediate files
							> $resultdir/sc_es.txt
							> $resultdir/sc_es.txt_ld

							# this script will simulate genotypes and phenotypes
							Rscript simulate.R $K $NC $SC $CF $H 0 $((0+$AF)) $((0+$AF+$AF)) $AF $M lookup_chenling_inter.txt $resultdir/sc_es.txt $ITER params_lookup.txt >> $resultdir/results.txt
							...
							# - again phenotypes are taken from a normal distribution
							...
							Rscript mleS.R $resultdir/sc_es.txt 0 >> $resultdir/results.txt
							Rscript mleS.R $resultdir/sc_es.txt_ld 0 >> $resultdir/results.txt
							...
							echo >> $resultdir/results.txt # add new line

							rm $resultdir/sc_es.* # uncomment if you want to clean up
						done
					done
				done
			done
...
Rscript plotRes.R lookup_chenling.txt $resultdir/results.txt
